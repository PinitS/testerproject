...
<div class="row mt-3">
    ...
    <div class="col-md-12 text-center">
        ...
        <div class="alert alert-danger">
            <h5 class="alert-heading"> {{Session::get('danger')}} </h5>
        </div>
        @elseif(Session::has('warning'))
        <div class="alert alert-warning">
            <h5 class="alert-heading"> {{Session::get('warning')}} </h5>
        </div>
        @elseif(Session::has('info'))
        <div class="alert alert-info">
            <h5 class="alert-heading"> {{Session::get('info')}} </h5>
        </div>
        @endif
...
<div class="row">
    <div class="col-md-6 mt-2">
        ...
        <a href="{{route('cart.CustomShow' , ['cat_id' => 0 , 'usid' => Auth::user()->id ,'oid' => $oid , 'osetid'=> $osetid])}}"
            style='text-decoration:none;'>
            <button class="btn btn-danger mr-2" type="button">All</button>
        </a>

        @foreach ($categories as $category)
        <a href="{{route('cart.CustomShow' , ['cat_id' => $category->id , 'usid' => Auth::user()->id ,'oid' => $oid , 'osetid'=> $osetid])}}"
            style='text-decoration:none;'>
    ...
    <div class="col-md-6 mt-2">
        <div class="input-group">
            <input type="text" class="form-control col-md-5 ml-auto" placeholder="Search for..."
                    ...
                    <div class="col-sm-6 col-md-3">
                        <div class="card">
                            <div class="card-header">
                                ...
                                @if(\Carbon\Carbon::today() >= $promotion->start && \Carbon\Carbon::today() <=
                                    $promotion->end)
                                    {{-- <strong class="h4 ml-3"> {{$promotion->discount}} </strong> --}}
                                    <span class="badge badge-warning ml-2 mt-2">Promotion</span>
                                    @php
                                    ...
                                    <span class="badge badge-pill badge-success mt-2 float-right">New Product</span>
                                    @else
                                    <span class="badge badge-pill badge-danger mt-2 float-right">Recommended
                                        Product</span>
                                    @endif
                            ...
                            <div class="card-body">
                                ...
                                <div class="row">
                                    ...
                                    <button disabled class=" btn btn-sm btn-danger text-white mr-2 ml-auto"
                                        type="submit">
                                        Out of stock
                                    ...
                                    <a class="mr-2 ml-auto"
                                        href="{{route('cart.CustomStore' , ['pid' => $product->id , 'pname' => $product->name , 'usid' => Auth::user()->id , 'price' => $price  , 'promotion' => $textpromotion  , 'oid' => $oid , 'osetid'=> $osetid])}}"
                                        style='text-decoration:none;'>
...
<div class="card card-accent-danger pnt-fix"
    style="position: fixed; top: 89px; right: 30px; width: 34rem; display: none;">
    <div class="card-header">
    ...
    <div class="card-body">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="btn-group" role="group" aria-label="Basic example">
                                    @if($Cart->quatity <= 1) @php $des_minus="disabled" ; @endphp @else @php
                                        $des_minus="" ; @endphp @endif @if($Cart->quatity >=
                                        $Cart->productinfo->count_quatity && $Cart->productinfo->active_count == 1)
                                        ...
                                        <form
                                            action="{{ action('CartController@UpdateQuatity' , ['id' => $Cart->id , 'usid' => Auth::user()->id , 'oid' => $oid , 'osetid'=> $osetid]  )}}"
                                            method="post">
                                            {{ csrf_field() }}
                                            ...
                                            <div class="btn-group" role="group" aria-label="Basic example">
                                                <button {{$des_minus}} class="btn btn-warning btn-sm text-white"
                                                    name="Value" value="0" type="submit"><i
                                                        class="fas fa-minus fa-1x"></i></button>
                                                <strong class="text-dark mr-3 ml-3 mt-1"> {{$Cart->quatity}} </strong>
                                                <button {{$des_plus}} class="btn btn-warning btn-sm text-white"
                                                    name="Value" value="1" type="submit"><i
                            ...
                            <td>
                                <div class="btn-group mr-1" role="group" aria-label="First group">
                                    <button class="btn btn-sm btn-secondary mb-1 mr-2 text-white" type="button"
                                        data-toggle="modal" data-target="#myModalmaterrial{{$Cart->id}}">
                                        <i class="fas fa-plus"></i>
                                    </button>

                                    <button class="btn btn-sm btn-info mb-1 mr-2 text-white" type="button"
                                        data-toggle="modal" data-target="#myModalhastag{{$Cart->id}}">
                                        <i class="fas fa-comment-alt"></i>
                                    </button>

                                    <a href="{{route('cart.CustomDelCart' , ['cat_id' => $Cart->id , 'usid' => Auth::user()->id ,'oid' => $oid , 'osetid'=> $osetid])}}"
                                        style='text-decoration:none;'>
                        ...
                        <tr>
                            ...
                            <td><strong class="text-danger">
                                    {{$Carts->sum('totalprice') + $cartDetails->sum('total_price')}} <i
                                        class="fab fa-btc"></i> </strong></td>
                            <td></td>
                        </tr>
                    </tbody>
        ...
        <div class="row">
            <div class="col-md-9 col-sm-12 mt-2">
                ...
                <button class="btn btn-block btn-primary mr-2" type="button" data-toggle="modal"
                    data-target="#largeModal">Confirm Order</button>

            </div>
            <div class="col-md-3 col-sm-12 mt-2">
                <a href="{{route('cart.CustomClear' , ['cat_id' => 0 , 'usid' => Auth::user()->id ,'oid' => $oid , 'osetid'=> $osetid])}}"
                    style='text-decoration:none;'>
        ...
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <form action="order" method="post">
                    ...
                    <input type="hidden" id="osetid" name="osetid" value="{{$osetid}}">
...
<form action="cartDetail" method="post">
    ...
    <div class="modal fade" id="myModalmaterrial{{$Cart->id}}" tabindex="-1" role="dialog"
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <input type="hidden" id="materialprice" name="materialprice[{!! $material->id !!}]"
                                        value="{{$material->materialprice}}">

                                    <input type="hidden" id="user_id" name="user_id" value="{{Auth::user()->id}}">
                                    <input type="hidden" class="material_id_value{!! $material->id !!}"
                                        name="material_id_value[{!! $material->id !!}]" value="{!! $chk !!}">

                                    <label class="c-switch c-switch-label c-switch-danger">
                                        <input {{ $chk == 'true' ? 'checked=""' : '' }} type="checkbox"
                                            class="c-switch-input pnt-chk" id="material_id"
                                            data-id="{!! $material->id !!}" "> <span class=" c-switch-slider"
...
{{-- modal hastag --}}
@foreach($Carts as $Cart)
<form action="/Updatehashtag+{{$Cart->id}}+{{$oid}}+{{$osetid}}" method="get">
    <div class="modal fade" id="myModalhastag{{$Cart->id}}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
        aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <div class="modal-body">
                    ...
                    <button type="button" class="btn btn-lg btn-danger mb-2 pnt-add-text{{$hashtag->id}}"
                        data-id="{{$hashtag->id}}"
                        data-value="{{$hashtag->hashtagname}}">#{{$hashtag->hashtagname}}</button>
                    @endforeach
                    <input type="hidden" id="user_id" name="user_id" value="{{Auth::user()->id}}">
                    <input class="form-control pnt-input-text" id="hashtagname" name="hashtagname" type="text"
                        placeholder="Enter Hashtag" value="{{$Cart->note}}" required>
                </div>
                <div class="modal-footer">
                    <button class="btn btn-secondary" type="button" data-dismiss="modal">Close</button>
                    <button class="btn btn-primary" type="submit">Save</button>
